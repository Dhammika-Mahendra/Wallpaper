...
export default function Colorbar({colList,setColList,range,setRange,check}) {
  ...
  const setCol=(e)=>{ 
    ...
    setColList(colList.map((el)=>{
      if(e.target.id==el.id){return {id:Number(e.target.id),c:el.c,R:r,G:g,B:b}}else{return el}
    }))
...
const [Navig,setNavig]=useState({xCoord:'',yCoord:''});
...
const linear_str=()=>{
  ...
  if(!check.status){
    let str='linear-gradient(to top'
    colList.forEach((elem,indx) => {
      let point=Math.floor(elem.c*100/300)
    ...
    let dist=((300/check.groups)/300)*100
    let colGap=1/(check.groups-1)
    let point=0;
    let x,y,z
    ...
    <div style={{width:"auto",display:'flex',flexDirection:'column',alignItems:'center'}}>
       ...
       <div style={{width:'100%',display:'flex',justifyContent:'center'}}>
         <div 
             ...
             onMouseMove={(e)=>setNavig({xCoord:e.clientX - e.target.getBoundingClientRect().left,yCoord:e.clientY - e.target.getBoundingClientRect().top})}  onMouseLeave={()=>setNavig({...Navig,xCoord:0})}>
       ...
       <div style={{marginBottom:'10px',marginTop:'5px',width:'100px',display:'flex',flexDirection:'column',alignItems:'center'}}>
        ...
        ></input>
        <input size='small' type='number' style={{width:"60px",height:"20px",border:'1px solid gray',borderRadius:'5px',backgroundColor:'#fafafa',textAlign:'right'}}
